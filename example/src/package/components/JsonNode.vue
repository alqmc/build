<template>
  ...
  >
    ...
    <span :class="['json-node-value']">
      ...
      <span v-if="isSub && isExtend && !disabled && !valueSlot" class="json-node-icon-add" @click="handleAddAttr()">
        <IconSvg fix-class="json-node-operate-icon" name="icon-tianjia"></IconSvg>
      </span>
      <span v-if="isSub && !isExtend && !valueSlot">......</span>
      <slot v-if="valueSlot" name="node-value" :nodeValue="stateData" :handleChange="handleSlotValueChange"></slot>
      <ValueEditor
      ...
      <span v-if="!disabled && !isExtend && !stateData.root && isHover" class="json-operate-warper">
        <slot name="type-switch" :allowType="allowType" :nodeValue="stateData">
          <select id="dataType" v-model="stateData.type" name="dataType" @change="handleSwitch(stateData)">
            <option v-for="datatype in allowType" :key="datatype.type" :value="datatype.type">
              {{ datatype.desc }}
            </option>
          </select>
        </slot>
      ...
      <template v-for="operate in customAppendOperate" :key="operate.key">
        ...
        >
          <Render v-if="operate.render" :render="operate.render"></Render>
...
import { _UUID, deepReductionJson, deepAnalysisJson, mergeArray, updataArrIndex } from '../utils';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleExtend = () => {
      ...
      eventTrigger('extend', { isExtend: isExtend.value, id: stateData.value?.id });
    };
    ...
    // 修改json
    const handleValueChange = (value: any, item: JsonItem, type: 'key' | 'value' = 'value') => {
      item[type] = value;
    ...
    const handleAttrDelete = () => {
      ...
      eventTrigger('extend', { isExtend: false, id: stateData.value?.id });
    };
